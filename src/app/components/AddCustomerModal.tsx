...
export default function AddCustomerModal({ customer, onChange, onSave, onClose }: AddCustomerModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-start justify-center overflow-y-auto p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:p-6 lg:p-8" onClick={onClose} role="presentation">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-auto overflow-hidden" onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="customer-modal-title">
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormItem label="Full Name *">
                <input className="h-11 px-3 text-sm border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-brand-500 outline-none w-full" value={customer.name || ''} onChange={e => set('name', e.target.value)} placeholder="e.g. John Smith" autoFocus />
              </FormItem>
              <FormItem label="Email *">
                <input className="h-11 px-3 text-sm border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-brand-500 outline-none w-full" type="email" value={customer.email || ''} onChange={e => set('email', e.target.value)} placeholder="e.g. john@company.com" />
              </FormItem>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormItem label="Phone *">
                <input className="h-11 px-3 text-sm border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-brand-500 outline-none w-full" value={customer.phone || ''} onChange={e => set('phone', e.target.value)} placeholder="e.g. +263 77 123 4567" />
              </FormItem>
              <FormItem label="WhatsApp">
                <input className="h-11 px-3 text-sm border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-brand-500 outline-none w-full" value={customer.whatsapp || ''} onChange={e => set('whatsapp', e.target.value)} placeholder="Same as phone if identical" />
              </FormItem>
            </div>

            <FormItem label="Billing Address *">
              <input className="h-11 px-3 text-sm border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-brand-500 outline-none w-full" value={customer.address || ''} onChange={e => set('address', e.target.value)} placeholder="e.g. 123 Main Street, Harare" />
            </FormItem>

            <FormItem label="Site Address" helper="Leave blank if the same as billing address.">
              <input className="h-11 px-3 text-sm border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-brand-500 outline-none w-full" value={customer.siteAddress || ''} onChange={e => set('siteAddress', e.target.value)} placeholder="e.g. 456 Factory Road, Industrial Area" />
            </FormItem>
          </div>
